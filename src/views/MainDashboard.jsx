...
function MainDashboard() {
    ...
    if (isSpectrometerConnected) {
      ...
      try {
        ...
        if (error.response?.status === 500) {
          errorMessage = 'Server error: Please check if the spectrometer service is running';
  ...
  const handleConfigUpdate = async () => {
    ...
    try {
      console.log('Updating config with:', { selectedGrade, incorrectElementsCount });
      ...
      if (error.response?.status === 500) {
        errorMessage = 'Server error: Please check if the configuration endpoint is working properly';
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-sm p-6 h-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label htmlFor="metal-grade-select" className="block text-sm font-medium text-gray-700 mb-2">
                    Metal Grade
                  </label>
                  <select
                  ...
                  >
                    <option value="">
                      {isLoadingGrades ? 'Loading grades...' : 'Select a metal grade'}
                    </option>
                ...
                <div>
                  <label htmlFor="incorrect-elements-input" className="block text-sm font-medium text-gray-700 mb-2">
                    Incorrect Elements Count
                  </label>
                  <input
                    ...
                    onChange={(e) => setIncorrectElementsCount(parseInt(e.target.value) || 0)}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
              ...
              {selectedGradeDetails && (
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    {Object.entries(selectedGradeDetails.composition_range).map(([element, range]) => (
                      <div key={element} className="bg-white p-3 rounded-md text-center">
              ...
              <div className="mt-6">
                <button
                  ...
                  disabled={!selectedGrade || isUpdatingConfig || !isSpectrometerConnected}
                  className={`w-full py-3 px-4 rounded-lg font-semibold transition-colors ${
          ...
          {spectrometerData ? (
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-gray-200">
                <div>
                  <h3 className="text-lg font-medium text-gray-800">Latest Reading</h3>
                  <p className="text-sm text-gray-600">
                    Received: {new Date(spectrometerData.timestamp).toLocaleString()}
                  </p>
                </div>
              ...
              {/* Composition Data */}
              <div>
                <h4 className="text-md font-semibold text-gray-700 mb-3">Composition Analysis</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-4">
                  {(() => {
                    const reading = parseSpectrometerReading(spectrometerData.reading);
                    return reading ? Object.entries(reading).map(([element, value]) => (
                      <div key={element} className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg text-center border border-blue-200">
                        <div className="text-lg font-bold text-blue-800">{element}</div>
                        <div className="text-xl font-semibold text-blue-900">{value}%</div>
                        {selectedGradeDetails && selectedGradeDetails.composition_range[element] && (
                          <div className="text-xs text-blue-600 mt-1">
                            Target: {selectedGradeDetails.composition_range[element][0]}-{selectedGradeDetails.composition_range[element][1]}%
                          </div>
                        )}
                      </div>
                    )) : (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 max-w-md mx-auto">
                Connect to the spectrometer and configure settings to start receiving real-time data. 
                Data will appear here automatically when readings are generated.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-gray-500 mb-2">AI Analysis Coming Soon</h3>
            <p className="text-gray-400 max-w-md mx-auto">
              Once spectrometer data is available, our AI will analyze the composition and provide 
              intelligent alloy addition recommendations to meet your target specifications.
            </p>
          </div>
        </div>
      </div>
